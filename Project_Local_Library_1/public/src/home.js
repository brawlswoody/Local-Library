...
function getMostCommonGenres(books) {
    ...
    return Object.entries(genre).map(([name, count]) => { return { name, count } }).sort((a, b) => b.count - a.count).slice(0, 5);
}
...
function getMostPopularAuthors(books, authors) {
    ...
    books.forEach((book) => {
        let foundAuthor = finalArr.find((author) => { return author.id === book.authorId });
        foundAuthor.count += book.borrows.length;
    })

    return finalArr.sort((authorA, authorB) => authorB.count - authorA.count).slice(0, 5).map(({ id, ...rest }) => rest)
}
